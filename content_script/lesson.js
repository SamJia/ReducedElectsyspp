...
function optimize_elect() {
	...
	*/
	...
	jQuery("[name=myradiogroup]", document).slice(0, 1).parent().parent().parent().prev().children().slice(0, 1).css({
...
function processLidQueue() {
	...
	var data = {};
	data = {
		"__VIEWSTATE": jQuery('#__VIEWSTATE', document).val(),
		"__EVENTVALIDATION": jQuery('#__EVENTVALIDATION', document).val()
	}
	if (type == "renxuan") {
...
function draw(weekday, hour_from, hour_to, isFull, lid, clicked, t_order) {
	...
	jQuery("." + draw_id + "_small", document).css({
		"width": draw_width - 3,
		"height": draw_height - 3,
		"position": "absolute",
		"top": posy + 25 + 2,
